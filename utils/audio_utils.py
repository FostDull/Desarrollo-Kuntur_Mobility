...
from base.MongoKr import guardar_evidencia
...
def tiene_audio(video_path):
    ...
    try:
        ...
        logger.error(f"Error verificando audio en video: {e}")
        return False

def extract_audio(video_path, audio_path):
    """Extraer audio de un video usando FFmpeg"""
    if not tiene_audio(video_path):
        logger.warning(f"⚠️ El video no tiene pista de audio: {video_path}")
        return False
    try:
        cmd = [
            "ffmpeg",
...
def procesar_audio(video_path, visual_json_path):
    ...
    if not extract_audio(video_path, audio_path):
        logger.warning(f"No se extrajo audio de: {file_name}. Se continuará sin transcripción.")
        transcription = ""
    else:
