...
def subir_video_b2(video_path, nombre_archivo, key_id=None, app_key=None, bucket_id=None):
    """Sube un video a Backblaze B2."""

    # Leer desde .env si no se pasa como parámetro
    key_id = key_id or os.getenv("B2_KEY_ID")
    app_key = app_key or os.getenv("B2_APP_KEY")
    bucket_id = bucket_id or os.getenv("B2_BUCKET_ID")

    if not all([key_id, app_key, bucket_id]):
        logger.error("❌ Faltan credenciales de Backblaze. Verifica .env o parámetros.")
        return False
    ...
    # 2. Validar bucket_id antes de usarlo
    if not bucket_id:
        logger.error("❌ Bucket ID no proporcionado.")
        return False
    ...
    try:
        ...
        logger.info(f"Payload que se enviará: {payload}")
